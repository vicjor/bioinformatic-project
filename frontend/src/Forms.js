...
export default function Forms(props){
    ...
    const getPwm = () => {
        if(chosenMatrix != 'no chosen matrix'){
            get_data(`http://localhost:8000/pwm/${chosenMatrix}`, setPwm)
        }
    }
    ...
    return(
        <div>
            ...
            {props.progress === 0 ? 
                <div>
                    { matrices !== 0 ? matrices['matrices_ids'].map((key, index) => <p onClick={() => setChosenMatrix(key)}>{key}</p>) : <p>Laster...</p>}
                </div> 
                : props.progress === 1 ? 
                <div>
                    <p>Her kommer egt "skriv inn DNA sekvens". Har bare denne midlertidig.</p><br />
                    { pwm !== 0 ? <h3>PWM matrix for {chosenMatrix}</h3> : null}
                    { pwm !== 0 ? Object.keys(pwm['pwm']).map((key, index) => 
                        <p>
                            {pwm['pwm'][key].map((value, index) => value+ ', ' )} 
                        </p>
                    ) : <p>laster ...</p>}
                </div>
                : props.progress === 2 ?
                <div><p>Her kommer bakgrunnsfordeling</p></div>
                : null}
